...
namespace MemoryVectorDB
{
    ...
    public class VectorDB<T> where T : IVectorObject
    {
        ...
        public List<T> VectorObjects = new();
        ...
        public async Task AddAsync(T vectorObject)
        {
            ...
            {
                VectorObjects.Add(vectorizedObject);
            }            
        }
        ...
        public SortedList<float, T> FindNearestSorted(float[] query, int noItems)
        {
            ...
            for (int i = 0; i < VectorObjects.Count; i++)
            {
                ...
                float dotProduct = DotProduct(VectorObjects[i].GetVector(), query);
                  
                if (dotProduct > maxDotProduct)
                {
                    // Add to the list
                    nearestObjects.Add(dotProduct, VectorObjects[i]);
        ...
        public async Task DeserializeJsonAsync(string path)
        {
            ...
            if (data?.VectorObjects != null) { VectorObjects = data.VectorObjects; }
        }
    }
}